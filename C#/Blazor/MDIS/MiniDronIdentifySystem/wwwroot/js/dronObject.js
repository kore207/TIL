...
function addDronMarker(obj) {
    ...
    let dronFeature = new ol.Feature({
        ...
        contents: //팝업 내부에 표시되는 부분
            ...
            + '<hr>'
            + '<button type="button" class="btn btn-primary" id="dronInfoPop" onclick="roadDronInfomation()";>드론정보</button>' + '&nbsp&nbsp'
...
let roadDronInfomation = function () {    
    $.ajax({
        type: "get",
        url: "http://localhost:56728/api/drones/" + selected.values_.id.slice(5),
        dataType: 'json',
        async: false,        
        //jsonpCallback: "callback", //jsonp 로 받는경우 date 는 다시 파싱해줘야한다. 
        success: function (data) {
            //data.CreateTime = new Date(parseInt(data.CreateTime.substr(6))).toString();
            //data.UpdateTime = new Date(parseInt(data.UpdateTime.substr(6))).toString();
            console.log(data);
        },
        error: function (xhr, stat, err) {
